...
def generate_dataset(path_source, path_dest, rho, height, width, data, box, overlap):
    successful_count = 0
    attempt_count = 0
    max_attempts = data * 3  # 最大尝试次数

    while successful_count < data and attempt_count < max_attempts:
        attempt_count += 1
        ...
        convex_attempts = 0
        while convex_attempts < 100:  # 限制凸四边形尝试次数
            convex_attempts += 1
            for j in range(8):
        ...
        # Generate input1 - 添加边界检查
        x1 = int(src_input1[0][0])
        y1 = int(src_input1[0][1])
        # 检查input1是否在边界内
        if x1 < 0 or y1 < 0 or x1 + box > width or y1 + box > height:
            continue

        image1 = img[y1:y1 + box, x1:x1 + box]

        # Generate input2 - 添加边界检查
        x2 = int(src_input2[0][0])
        y2 = int(src_input2[0][1])
        # 检查input2是否在边界内
        if x2 < 0 or y2 < 0 or x2 + box > width or y2 + box > height:
            continue
        ...
        # 检查图像尺寸是否正确
        if image1.shape[0] != box or image1.shape[1] != box:
            image1 = cv2.resize(image1, (box, box))
        if image2.shape[0] != box or image2.shape[1] != box:
            image2 = cv2.resize(image2, (box, box))

        save_to_file_wolabel(str(successful_count + 1).zfill(6), image1, image2, path_dest, gt_4pt_shift)
        successful_count += 1
        print(f"成功生成: {successful_count}/{data}")

    print(f"生成完成: {successful_count} 个样本，共尝试 {attempt_count} 次")


raw_image_path_train = 'gdata/True'
...
dataset_size = 500
generate_image_path = 'gdata/test'
