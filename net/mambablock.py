...
class RepDW(torch.nn.Module):
    def __init__(self, ed) -> None:
        super().__init__()
        # 深度可分离卷积层  同时包含卷积和批量归一化（BatchNorm）操作
...
class LowSS2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor, nrows=-1, channel_first=False):
        ...
        x_low = F.interpolate(x_low, scale_factor=2, mode='bilinear') + res
...
class MLPBlock(nn.Module):
    ...
    def __init__(self,
        ...
        mlp_layer: List[nn.Module] = [
            Conv2d_BN(dim, mlp_hidden_dim, 1),
            act_layer(),
            Conv2d_BN(mlp_hidden_dim, dim, 1)
        ]
...
class MambaBlock(nn.Module):
    def __init__(
        ...
        self.in_proj = Conv2d_BN(hidden_dim,hidden_dim, 1)
        self.out_proj = Conv2d_BN(hidden_dim,hidden_dim, 1)
        ...
        self.bn = torch.nn.BatchNorm2d(hidden_dim)
    ...
    def forward(self, input: torch.Tensor):
        x = input + self.drop_path(self.ss2d(self.bn(input)))
        x = x + self.drop_path(self.mlp(self.bn(x)))  # FFN
        return x
