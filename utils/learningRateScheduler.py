...
def warmUpLearningRate(max_num_epochs,warm_up_epochs=1,scheduler='cosine',initial_lr=2e-4, min_lr=1e-6):
    lr_milestones = [20, 40]
    ...
    if scheduler == 'multistep':
        ...
        finnalScheduler = lambda epoch: (epoch + 1) / warm_up_epochs if epoch < warm_up_epochs \
            else max(min_lr / initial_lr, 0.5 * (math.cos((epoch - warm_up_epochs) / (max_num_epochs - warm_up_epochs) * math.pi) + 1))
